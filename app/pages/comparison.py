...
data_argenx = st.session_state['data_argenx']
data_gmab = st.session_state['data_gmab']
data_overview = st.session_state['data_overview']

with st.sidebar:
    selected_company_names = st.multiselect(
        "Select Companies:",
        options=sorted(data_overview['symbol'].unique())
    )
